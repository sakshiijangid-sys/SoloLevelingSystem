    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none select-none overflow-hidden rounded-t-2xl xs:rounded-t-3xl sm:rounded-t-[2.5rem]">
        <img
          ...
          className="w-full h-full object-cover object-[15%_center] opacity-15 dark:opacity-30 mix-blend-multiply dark:mix-blend-screen transform scale-120 -translate-x-12 sm:-translate-x-20 md:-translate-x-32 transition-transform duration-300"
        />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 xs:px-6 sm:px-8 py-5 sm:py-6 min-h-[380px] sm:h-[490px] flex flex-col justify-between overflow-visible">
        ...
        <div className="flex flex-col md:flex-row items-start justify-between gap-6 overflow-visible w-full flex-1 pt-1 sm:pt-2 pb-4">
          
          {/* Asymmetric Add to Home Screen Callout - Positioned at Top */}
          <motion.div 
            id="footer-homescreen-prompt"
            initial={{ opacity: 0, y: -8 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            className="self-start max-w-full xs:max-w-[320px] sm:max-w-[400px] md:max-w-[480px] text-left select-none pointer-events-auto group"
          >
            <div className="flex flex-col items-start gap-1 sm:gap-1.5 p-0 bg-transparent transition-all duration-300">
              
              {/* Line 1: Add / Install Badge */}
              <div className="flex items-center gap-2">
                <span className="font-heading text-[10px] sm:text-xs tracking-wider uppercase text-purple-700 dark:text-purple-300 font-semibold">
                  Get The Official App
                </span>
                <span className="hidden xs:inline-block w-1 h-1 rounded-full bg-amber-500/80" />
                <span className="hidden xs:inline-block font-mono text-[9px] sm:text-[10px] text-zinc-500 dark:text-zinc-400 uppercase tracking-widest">
                  Android APK & PWA
                </span>
              </div>

              {/* Line 2: Solo Leveling System */}
              <h2 className="font-heading text-sm xs:text-base sm:text-lg md:text-xl font-bold tracking-wide uppercase text-zinc-950 dark:text-white drop-shadow-[0_0_12px_rgba(251,191,36,0.65)] leading-tight py-0.5">
                Solo Leveling System
              </h2>

              {/* Line 3: Dynamic wording adjusted for mobile/desktop layout */}
              <p className="font-sans text-[11px] xs:text-xs sm:text-sm tracking-wide uppercase font-medium text-zinc-700 dark:text-zinc-300 leading-snug">
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 pt-1.5">
                ...
                {onInstallClick && (
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg border border-zinc-400/40 dark:border-zinc-700/60 hover:border-purple-400/50 bg-zinc-200/50 dark:bg-zinc-800/50 hover:bg-purple-600/10 text-zinc-800 dark:text-zinc-300 hover:text-purple-600 dark:hover:text-purple-200 font-mono text-[10px] sm:text-xs font-medium uppercase tracking-wider transition-all active:scale-95 cursor-pointer w-auto"
                    title="Install as Progressive Web App"
                  >
                    <Smartphone className="w-3 h-3 text-purple-500 dark:text-purple-400 shrink-0" />
                    <span>PWA GUIDE</span>
                  </button>
                )}
              </div>
          ...
          <div className="self-end md:self-start -mt-20 xs:-mt-24 sm:-mt-28 md:-mt-32 -translate-y-4 sm:-translate-y-8 relative z-30 filter drop-shadow-[0_-10px_24px_rgba(168,85,247,0.35)]">
        ...
        <div className="mt-auto pt-3 border-t border-gray-200/60 dark:border-zinc-800/60 flex flex-col sm:flex-row items-center justify-between gap-2 text-[11px] font-mono text-gray-500 dark:text-zinc-400">
          <div className="flex items-center gap-2">
            ...
            <span className="hidden sm:inline">• REWARDS SECURED</span>
          </div>
        </div>
